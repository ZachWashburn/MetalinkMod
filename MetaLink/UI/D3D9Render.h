...
template<class T>
const T& our_clamp(const T& x, const T& upper, const T& lower) {
	return min(upper, max(x, lower));
} // This sometimes doesnt work?
#include <math.h>
#include <algorithm>
...
public:
	...
	bool Init(RendererUtils::RendererInitializer* RI) {
		...
		if (!v_buffer) {
			...
			throw std::exception(XorStr("D3D9Renderer::INIT Failed to create Vertex Buffer"));
			return true;
		}

		//const char* fontName = "zorque";
		//const char* fontName = "tahoma";
		const char* fontName = XorStr("constanb");
		ID3DXFont* pfont; 
