...
{
	return GetValveAllocator()->RegionAlloc(MEMALLOC_REGION, nSize);
}

inline void *MemAlloc_Alloc(size_t nSize, const char *pFileName, int nLine)
{
	return GetValveAllocator()->RegionAlloc(MEMALLOC_REGION, nSize, pFileName, nLine);
}
#else
...
{
	GetValveAllocator()->Free(ptr);
}
inline void MemAlloc_Free(void *ptr, const char *pFileName, int nLine)
{
	GetValveAllocator()->Free(ptr, pFileName, nLine);
}
...
inline void *MemAlloc_AllocAligned(size_t size, size_t align)
{
	...
	if ((pAlloc = (unsigned char*)GetValveAllocator()->Alloc(sizeof(void *) + align + size)) == (unsigned char*)NULL)
		return NULL;
...
inline void *MemAlloc_AllocAligned(size_t size, size_t align, const char *pszFile, int nLine)
{
	...
	if ((pAlloc = (unsigned char*)GetValveAllocator()->Alloc(sizeof(void *) + align + size, pszFile, nLine)) == (unsigned char*)NULL)
		return NULL;
...
inline void MemAlloc_FreeAligned(void *pMemBlock, const char *pFileName, int nLine)
{
	...
	// pAlloc is the pointer to the start of memory block
	pAlloc = *((void **)pAlloc);
	GetValveAllocator()->Free(pAlloc, pFileName, nLine);
}
...
inline size_t MemAlloc_GetSizeAligned(void *pMemBlock)
{
	...
	// pAlloc is the pointer to the start of memory block
	pAlloc = *((void **)pAlloc);
	return GetValveAllocator()->GetSize(pAlloc) - ((unsigned char *)pMemBlock - (unsigned char *)pAlloc);
}
...
#define MemAlloc_PushAllocDbgInfo( pszFile, line ) GetValveAllocator()->PushAllocDbgInfo( pszFile, line )
#define MemAlloc_PopAllocDbgInfo() GetValveAllocator()->PopAllocDbgInfo()
#define MemAlloc_RegisterAllocation( pFileName, nLine, nLogicalSize, nActualSize, nTime ) GetValveAllocator()->RegisterAllocation( pFileName, nLine, nLogicalSize, nActualSize, nTime )
#define MemAlloc_RegisterDeallocation( pFileName, nLine, nLogicalSize, nActualSize, nTime ) GetValveAllocator()->RegisterDeallocation( pFileName, nLine, nLogicalSize, nActualSize, nTime )
#else
...
#define MemAlloc_DumpStats() GetValveAllocator()->DumpStats()
#define MemAlloc_CompactHeap() GetValveAllocator()->CompactHeap()
#define MemAlloc_OutOfMemory() GetValveAllocator()->OutOfMemory()
#define MemAlloc_CompactIncremental() GetValveAllocator()->CompactIncremental()
#define MemAlloc_DumpStatsFileBase( _filename ) GetValveAllocator()->DumpStatsFileBase( _filename )
#define MemAlloc_CrtCheckMemory() GetValveAllocator()->CrtCheckMemory()
#define MemAlloc_GlobalMemoryStatus( _usedMemory, _freeMemory ) GetValveAllocator()->GlobalMemoryStatus( _usedMemory, _freeMemory )
#define MemAlloc_MemoryAllocFailed() GetValveAllocator()->MemoryAllocFailed()

#define MemAlloc_GetDebugInfoSize() GetValveAllocator()->GetDebugInfoSize()
#define MemAlloc_SaveDebugInfo( pvDebugInfo ) GetValveAllocator()->SaveDebugInfo( pvDebugInfo )
#define MemAlloc_RestoreDebugInfo( pvDebugInfo ) GetValveAllocator()->RestoreDebugInfo( pvDebugInfo )
#define MemAlloc_InitDebugInfo( pvDebugInfo, pchRootFileName, nLine ) GetValveAllocator()->InitDebugInfo( pvDebugInfo, pchRootFileName, nLine )
#define MemAlloc_GetSize( x ) GetValveAllocator()->GetSize( x );
//-----------------------------------------------------------------------------
...
#define MEMALLOC_DEFINE_EXTERNAL_TRACKING( tag ) \
	...
	static const char *g_psz##tag##Alloc = strcpy( (char *)GetValveAllocator()->Alloc( strlen( #tag "Alloc" ) + 1, "intentional leak", 0 ), #tag "Alloc" );
...
#define MemAlloc_RegisterExternalAllocation( tag, p, size ) \
	...
	{ \
		...
		GetValveAllocator()->GetActualDbgInfo( fileLine.pszFile, fileLine.line ); \
...
#ifndef NO_MALLOC_OVERRIDE //dylan added to fix compile
#define MemAlloc_GetDebugInfoSize() GetValveAllocator()->GetDebugInfoSize()
#define MemAlloc_SaveDebugInfo( pvDebugInfo ) GetValveAllocator()->SaveDebugInfo( pvDebugInfo )
#define MemAlloc_RestoreDebugInfo( pvDebugInfo ) GetValveAllocator()->RestoreDebugInfo( pvDebugInfo )
#define MemAlloc_InitDebugInfo( pvDebugInfo, pchRootFileName, nLine ) GetValveAllocator()->InitDebugInfo( pvDebugInfo, pchRootFileName, nLine )
...
#else
__forceinline void MemoryLogMessage(char const *s)
{
}

__forceinline void EnableMemoryLogging(bool bOnOff)
{
}
__forceinline void DumpMemoryLog(int nThresh)
{
}
__forceinline void DumpMemorySummary(void)
{
}
__forceinline void SetMemoryMark(void)
{
}
__forceinline void DumpChangedMemory(int nThresh)
{
}
...
#else
__forceinline size_t ApproximateProcessMemoryUsage(void)
{
	return 0;
}
