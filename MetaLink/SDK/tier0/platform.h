...
inline bool Plat_IsInDebugSession() { return IsDebuggerPresent(); };
...
// It is hilarious that inline basically means global in c++20 so you can do inline __declspec(nonline)
typedef double(__cdecl* Plat_FloatTimeFunc_t)();
inline Plat_FloatTimeFunc_t Plat_FloatTime;
#if 0
inline  __declspec(noinline) double			Plat_FloatTime() { 
	static Plat_FloatTimeFunc_t Plat_FloatT{ (Plat_FloatTimeFunc_t)GetProcAddress(GetModuleHandleA(XorStr("tier0")), XorStr("Plat_FloatTime")) };
	if(Plat_FloatT)
		return Plat_FloatT();
	return 0;
...
inline void				Plat_ExitProcess(int nCode) { exit(0);  };
