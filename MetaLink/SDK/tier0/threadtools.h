...
public:
	...
	// Start thread running  - error if already running
	virtual bool Start(unsigned nBytesStack = 0, ThreadPriorityEnum_t nPriority = TP_PRIORITY_DEFAULT) { return false; };
	...
	// is no longer alive.
	bool Join(unsigned timeout = TT_INFINITE) { return false; };

	// Access the thread handle directly
	ThreadHandle_t GetThreadHandle() { return m_hThread; };

#ifdef _WIN32
	uint GetThreadId() { return 0; };
#endif
	...
	// Set the priority
	bool SetPriority(int priority) { return false; };
	...
	unsigned Resume(){ return 0;}

	// Check if thread is suspended
	bool IsSuspended() { throw; }

	// Force hard-termination of thread.  Used for critical failures.
	bool Terminate(int exitCode = 0) { return false; };
...
protected:
	...
	virtual bool WaitForCreateComplete(CThreadEvent* pEvent) { return false; };
	...
	virtual ThreadProc_t GetThreadProc() { return (ThreadProc_t)0; };
