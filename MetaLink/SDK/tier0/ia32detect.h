...
#	pragma pack(pop)
	...
	ia32detect ()
	{
		...
		if (m >= 2)
			init2((std::byte)d[4] & (std::byte)0xFF);
...
protected:
	...
	const tstring brand_text () const
	{
		...
		if ((std::byte)misc.Brand < (std::byte)brand_invalid)
			return tstring(text[(int)misc.Brand]);
		else
		{
			tchar b[32];
...
private:
	...
	void init2 (std::byte count)
	{
		...
		cache = new std::byte[m];

		m = 0;

		for (int ci3 = 1; ci3 < 256; ci3++)
			if (c[ci3])
				cache[m++] = (std::byte)ci3;
