...
#define ASSERT(COND) _ASSERT(COND)
#define REMOVED_FOR_COMPILE Assert(0);
...
public:
	...
	Vector2D	VectorMin(const Vector2D &vOther) const;
	Vector2D	VectorMax(const Vector2D &vOther) const;
...
inline Vector2D Vector2D::VectorMin(const Vector2D &vOther) const
{
	return Vector2D(x < vOther.x ? x : vOther.x, 
		y < vOther.y ? y : vOther.y);
}

inline Vector2D Vector2D::VectorMax(const Vector2D &vOther) const
{
	return Vector2D(x > vOther.x ? x : vOther.x, 
		y > vOther.y ? y : vOther.y);
}
