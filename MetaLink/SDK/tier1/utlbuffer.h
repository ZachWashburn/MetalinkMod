...
public:
	...
	void			SetBufferType( bool bIsText, bool bContainsCRLF ) { throw; }

	// Makes sure we've got at least this much memory
	void			EnsureCapacity( int num ) { throw; }

	// Access for direct read into buffer
	void *			AccessForDirectRead( int nBytes );

	// Attaches the buffer to external memory....
	void			SetExternalBuffer( void* pMemory, int nSize, int nInitialPut, int nFlags = 0 ) { throw; }
	bool			IsExternallyAllocated() const;
	void			AssumeMemory( void *pMemory, int nSize, int nInitialPut, int nFlags = 0 ) { throw; }
	void			*Detach();
	void*			DetachMemory() { throw; }

	// copies data from another buffer
	void			CopyBuffer( const CUtlBuffer &buffer );
	void			CopyBuffer( const void *pubData, int cubData );

	void			Swap( CUtlBuffer &buf ) { throw; }
	void			Swap( CUtlMemory<uint8> &mem ) { throw; }
	...
	void			ActivateByteSwapping( bool bActivate ) { throw; }
	void			SetBigEndian( bool bigEndian ) { throw; }
	bool			IsBigEndian( void ) { throw; }
	...
	double			GetDouble( );
	void *			GetPtr();
	void			GetString(char* pString, int nMaxChars) { throw; };
	bool			Get(void* pMem, int size) {		throw;	};
	void			GetLine( char* pLine, int nMaxChars ){ throw; }
	...
	int				GetUpTo( void *pMem, int nSize ){ throw; }

	// This version of GetString converts \" to \\ and " to \, etc.
	// It also reads a " at the beginning and end of the string
	//int				GetUpTo(void* pMem, int nSize);
	void			GetDelimitedString( CUtlCharConversion *pConv, char *pString, int nMaxChars = 0 ) { throw; }
	char			GetDelimitedChar( CUtlCharConversion *pConv ) { throw; }
	...
	int				PeekDelimitedStringLength( CUtlCharConversion *pConv, bool bActualSize = true ) { throw; }

	// Just like scanf, but doesn't work in binary mode
	int				Scanf( SCANF_FORMAT_STRING const char* pFmt, ... ) { throw; }
 	int				VaScanf( const char* pFmt, va_list list ) { throw; }

	// Eats white space, advances Get index
	void			EatWhiteSpace() { throw; }

	// Eats C++ style comments
	bool			EatCPPComment() { throw; }
	...
	bool			ParseToken( const char *pStartingDelim, const char *pEndingDelim, char* pString, int nMaxLen ) { throw; }

	// Advance the get index until after the particular string is found
	// Do not eat whitespace before starting. Return false if it failed
	// String test is case-insensitive.
	bool			GetToken( const char *pToken ) { throw; }

	// Parses the next token, given a set of character breaks to stop at
	// Returns the length of the token parsed in bytes (-1 if none parsed)
	int				ParseToken( characterset_t *pBreaks, char *pTokenBuf, int nMaxLen, bool bParseComments = true ) { throw; }
	...
	void			PutString( const char* pString ) { throw; }
	void			Put( const void* pMem, int size ) { throw; }
	...
	void			PutDelimitedString( CUtlCharConversion *pConv, const char *pString ) { throw; }
	void			PutDelimitedChar( CUtlCharConversion *pConv, char c ) { throw; }

	// Just like printf, writes a terminating zero in binary mode
	void			Printf( PRINTF_FORMAT_STRING const char* pFmt, ... ) FMTFUNCTION( 2, 3 ) { throw; }
	void			VaPrintf( const char* pFmt, va_list list ) { throw; }

	// What am I writing (put)/reading (get)?
	void* PeekPut( int offset = 0 );
	const void* PeekGet(int offset = 0) const;
	const void* PeekGet(int nMaxSize, int nOffset) { throw; }
	...
	void SeekPut( SeekType_t type, int offset ) { throw; }
	void SeekGet( SeekType_t type, int offset ) { throw; }
	...
	bool ConvertCRLF( CUtlBuffer &outBuf ) { throw; }
...
protected:
	...
	void SetOverflowFuncs( UtlBufferOverflowFunc_t getFunc, UtlBufferOverflowFunc_t putFunc ) { throw; }

	bool OnPutOverflow( int nSize ) { throw; }
	bool OnGetOverflow( int nSize ) { throw; }

protected:
	// Checks if a get/put is ok
	bool CheckPut( int size ) { throw; }
	bool CheckGet( int size ) { throw; }
	...
	void AddNullTermination( int nPut ) { throw; }
	...
	char GetDelimitedCharInternal( CUtlCharConversion *pConv ) { throw; }
	void PutDelimitedCharInternal( CUtlCharConversion *pConv, char c ) { throw; }

	// Default overflow funcs
	bool PutOverflow( int nSize ) { throw; }
	bool GetOverflow( int nSize ) { throw; }

	// Does the next bytes of the buffer match a pattern?
	bool PeekStringMatch( int nOffset, const char *pString, int nLen ) { throw; }

	// Peek size of line to come, check memory bound
	int	PeekLineLength() { throw; }

	// How much whitespace should I skip?
	int PeekWhiteSpace( int nOffset ) { throw; }

	// Checks if a peek get is ok
	bool CheckPeekGet( int nOffset, int nSize ) { throw; }

	// Call this to peek arbitrarily long into memory. It doesn't fail unless
	// it can't read *anything* new
	bool CheckArbitraryPeekGet( int nOffset, int &nIncrement ) { throw; }
