...
{
public:
	SUBFUNC();// virtual bool Connect(CreateInterfaceFn factory) = 0;
	SUBFUNC();// virtual void Disconnect() = 0;
	SUBFUNC();// virtual void* QueryInterface(const char* pInterfaceName) = 0;
	SUBFUNC();// virtual InitReturnVal_t Init() = 0;
	SUBFUNC();// virtual void Shutdown() = 0;
	SUBFUNC();// virtual CreateInterfaceFn	Init(char const* pShaderAPIDLL, IMaterialProxyFactory* pMaterialProxyFactory,CreateInterfaceFn fileSystemFactory,CreateInterfaceFn cvarFactory = NULL) = 0;
	SUBFUNC();// virtual void				SetShaderAPI(char const* pShaderAPIDLL) = 0;
	SUBFUNC();// virtual void				SetAdapter(int nAdapter, int nFlags) = 0;
	SUBFUNC();// virtual void				ModInit() = 0;
	SUBFUNC();// virtual void				ModShutdown() = 0;
	SUBFUNC();// virtual void				SetThreadMode(MaterialThreadMode_t mode, int nServiceThread = -1) = 0;
	SUBFUNC();// virtual MaterialThreadMode_t GetThreadMode() = 0;
	SUBFUNC();// virtual bool				IsRenderThreadSafe() = 0;
	SUBFUNC();// virtual void				ExecuteQueued() = 0;
	SUBFUNC();// virtual IMaterialSystemHardwareConfig* GetHardwareConfig(const char* pVersion, int* returnCode) = 0;
	SUBFUNC();// virtual bool				UpdateConfig(bool bForceUpdate) = 0;
	SUBFUNC();// virtual bool				OverrideConfig(const MaterialSystem_Config_t& config, bool bForceUpdate) = 0;
	SUBFUNC();// virtual const MaterialSystem_Config_t& GetCurrentConfigForVideoCard() const = 0;
	SUBFUNC();// virtual bool				GetRecommendedConfigurationInfo(int nDXLevel, KeyValues* pKeyValues) = 0;
	SUBFUNC();// virtual int					GetDisplayAdapterCount() const = 0;
	SUBFUNC();// virtual int					GetCurrentAdapter() const = 0;
	SUBFUNC();// virtual void				GetDisplayAdapterInfo(int adapter, MaterialAdapterInfo_t& info) const = 0;
	SUBFUNC();// virtual int					GetModeCount(int adapter) const = 0;
	SUBFUNC();// virtual void				GetModeInfo(int adapter, int mode, MaterialVideoMode_t& info) const = 0;
	SUBFUNC();// virtual void				AddModeChangeCallBack(ModeChangeCallbackFunc_t func) = 0;
	SUBFUNC();// virtual void				GetDisplayMode(MaterialVideoMode_t& mode) const = 0;
	SUBFUNC();// virtual bool				SetMode(void* hwnd, const MaterialSystem_Config_t& config) = 0;
	SUBFUNC();// virtual bool				SupportsMSAAMode(int nMSAAMode) = 0;
	SUBFUNC();// virtual const MaterialSystemHardwareIdentifier_t& GetVideoCardIdentifier(void) const = 0;
	SUBFUNC();// virtual void				SpewDriverInfo() const = 0;
	SUBFUNC();// virtual void				GetBackBufferDimensions(int& width, int& height) const = 0;
	SUBFUNC();// virtual ImageFormat			GetBackBufferFormat() const = 0;
	SUBFUNC();// virtual const AspectRatioInfo_t& GetAspectRatioInfo() const = 0;
	SUBFUNC();// virtual bool				SupportsHDRMode(HDRType_t nHDRModede) = 0;
	SUBFUNC();// virtual bool				AddView(void* hwnd) = 0;
	SUBFUNC();// virtual void				RemoveView(void* hwnd) = 0;
	SUBFUNC();// virtual void				SetView(void* hwnd) = 0;
	SUBFUNC();// virtual void				BeginFrame(float frameTime) = 0;
	SUBFUNC();// virtual void				EndFrame() = 0;
	SUBFUNC();// virtual void				Flush(bool flushHardware = false) = 0;
	SUBFUNC();// virtual uint32				GetCurrentFrameCount() = 0;
	SUBFUNC();// virtual void				SwapBuffers() = 0;
	SUBFUNC();// virtual void				EvictManagedResources() = 0;
	SUBFUNC();// virtual void				ReleaseResources(void) = 0;
	SUBFUNC();// virtual void				ReacquireResources(void) = 0;
	SUBFUNC();
	SUBFUNC();
	SUBFUNC();
	SUBFUNC();
	...
	virtual bool                IsMaterialLoaded(const char*) = 0;
	...
	SUBFUNC();
	SUBFUNC();
	SUBFUNC(); // virtual void				BeginLightmapAllocation() = 0;
	SUBFUNC(); // virtual void				EndLightmapAllocation() = 0;
	SUBFUNC(); // virtual void				CleanupLightmaps() = 0;
	SUBFUNC(); // virtual int 				AllocateLightmap(int width, int height,int offsetIntoLightmapPage[2],IMaterial* pMaterial) = 0;
	SUBFUNC(); // virtual int					AllocateWhiteLightmap(IMaterial* pMaterial) = 0;
	SUBFUNC(); // virtual void				UpdateLightmap(int lightmapPageID, int lightmapSize[2],int offsetIntoLightmapPage[2],float* pFloatImage, float* pFloatImageBump1,float* pFloatImageBump2, float* pFloatImageBump3) = 0;
	SUBFUNC(); // virtual int					GetNumSortIDs() = 0;
	SUBFUNC(); // virtual void				GetSortInfo(MaterialSystem_SortInfo_t* sortInfoArray) = 0;
	SUBFUNC(); // virtual void				GetLightmapPageSize(int lightmap, int* width, int* height) const = 0;
	SUBFUNC(); // virtual void				ResetMaterialLightmapPageInfo() = 0;
	SUBFUNC(); // virtual bool				IsStereoSupported() = 0;
	SUBFUNC(); // virtual bool				IsStereoActiveThisFrame() const = 0;
	SUBFUNC(); // virtual void				NVStereoUpdate() = 0;
	SUBFUNC(); // virtual void				ClearBuffers(bool bClearColor, bool bClearDepth, bool bClearStencil = false) = 0;
	SUBFUNC(); // virtual void SpinPresent(uint nFrames) = 0;
	SUBFUNC();
	virtual IMatRenderContext*  GetRenderContext() = 0;
	virtual void				BeginUpdateLightmaps(void) = 0;
	virtual void				EndUpdateLightmaps(void) = 0;
	virtual uint32				Lock() = 0;
...
public:
	...
	virtual uint32 CreateOcclusionQueryObject(void) = 0;
	virtual void DestroyOcclusionQueryObject(uint32) = 0;

	// Bracket drawing with begin and end so that we can get counts next frame.
	virtual void BeginOcclusionQueryDrawing(uint32) = 0;
	virtual void EndOcclusionQueryDrawing(uint32) = 0;

	// Get the number of pixels rendered between begin and end on an earlier frame.
	// Calling this in the same frame is a huge perf hit!
	virtual int OcclusionQuery_GetNumPixelsRendered(uint32) = 0;
	...
	virtual void ResetOcclusionQueryObject(uint32) = 0;
	...
	virtual void				PushDeformation(uint32 const* Deformation) = 0;
	...
	virtual uint32 AddLookup(const char* pName) = 0;
	virtual bool RemoveLookup(uint32 handle) = 0;
	virtual void LockLookup(uint32 handle) = 0;
	virtual void LoadLookup(uint32 handle, const char* pLookupName) = 0;
	virtual void UnlockLookup(uint32 handle) = 0;
	virtual void SetLookupWeight(uint32 handle, float flWeight) = 0;
	virtual void ResetLookupWeights() = 0;
	virtual void SetResetable(uint32 handle, bool bResetable) = 0;
	...
	virtual uint32 FindLookup(const char* pName) = 0;
};
