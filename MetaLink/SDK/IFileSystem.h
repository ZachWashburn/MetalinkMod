...
typedef std::uint32_t PathTypeQuery_t;
...
typedef void* FileAsyncRequest_t;
typedef void* FSAsyncControl_t;
typedef void* FSAsyncFile_t;
typedef void* WaitForResourcesHandle_t;

class IThreadedFileMD5Processor
{
public:
...
public:
	...
	virtual void			RemoveLoggingFunc(long logFunc) = 0;
	...
	virtual FSAsyncStatus_t	AsyncWriteFile(const char* pFileName, const void** pSrc, int nSrcBytes, bool bFreeMemory, bool bAppend = false, FSAsyncControl_t* pControl = NULL) = 0;
	...
	virtual FSAsyncStatus_t AsyncDirectoryScan(const char* pSearchSpec, bool recurseFolders, void* pContext, long pfnAdd, long pfnDone, FSAsyncControl_t* pControl = NULL) = 0;

	virtual bool			GetFileTypeForFullPath(char const* pFullPath, wchar_t* buf, size_t bufSizeInBytes) = 0;

	//--------------------------------------------------------
	//--------------------------------------------------------
	virtual bool		ReadToBuffer(FileHandle_t hFile, void* buf, int nMaxBytes = 0, FSAllocFunc_t pfnAlloc = NULL) = 0;
	...
	virtual void			RegisterFileWhitelist(void* pWantCRCList, void* pAllowFromDiskList, void** pFilesToReload) = 0;
	...
	virtual void			CacheFileCRCs(const char* pPathname, ECacheCRCType eType, void* pFilter) = 0;
	virtual EFileCRCStatus	CheckCachedFileHash(const char* pPathID, const char* pRelativeFilename, int nFileFraction, void* pFileHash) = 0;
	...
	virtual int				GetUnverifiedFileHashes(void* pFiles, int nMaxFiles) = 0;
